...
export default class ElanPlugin {
  ...
  parseElan(xml) {
    ...
    const timeSlots = xml.querySelectorAll(
      ...
      tier => ({
        ...
        annotations: _map.call(
          ...
          node => {
            ...
            if (
              this.Types.ALIGNABLE_ANNOTATION == annot.type
            ) {
              // Add start & end to alignable annotation
              annot.start =
                timeOrder[
                  node.getAttribute("TIME_SLOT_REF1")
                ];
              annot.end =
                timeOrder[
                  node.getAttribute("TIME_SLOT_REF2")
                ];
  ...
  render() {
    ...
    if (this.params.tiers) {
      tiers = tiers.filter(
        tier => tier.id in this.params.tiers
      );
    }
    ...
    tiers.forEach((tier, index) => {
      tier.annotations.forEach(annot => {
        if (
          annot.reference &&
          annot.reference.type ==
            this.Types.ALIGNABLE_ANNOTATION
        ) {
    ...
    // table
    const table = (this.table = document.createElement(
      "table"
    ));
